...
int s21_is_less_or_equal(s21_decimal value_1, s21_decimal value_2) {
    ...
    if (sign_val1 == sign_val2) {
        ...
        for (int i = 2; i > -1; i--) {
            flag = comparison_flag(flag, &value_1, &value_2, i,
            overflow_check);
            if (flag != 0) break;
        }
        if (sign_val1 == 0) {
            if (flag == 1 || flag == 0) bool_val = TRUE;
            if (flag == 2) bool_val = FALSE;
        } else {
            if (flag == 2 || flag == 0) bool_val = TRUE;
            if (flag == 1) bool_val = FALSE;
        }
    } else {
        if (sign_val1 == 1) {
            bool_val = TRUE;
